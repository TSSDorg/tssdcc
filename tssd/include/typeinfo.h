...
struct MkUniquePtr {
    using SharedPtr = [:T:];
    static SharedPtr New() {
        return std::make_unique<typename SharedPtr::element_type>();
    }
};

// SharePtr support share_ptr of unique_ptr
template <std::meta::info T, template <std::meta::info> class M>
...
public:
    ...
    TError dump(Buffer &buf, std::byte *dest) const override {
        ...
        if (-t == (std::int8_t)children_[0]->node_.tssd_type_) {
            buf.Dump(sizeof(t), (std::byte*)&t);
            if (*pshared_ptr)
                pshared_ptr->reset();
            return OK;
        }
